...
## 1. General Rules ^总则
To apply for XDAEX API, please follow the steps below in order to acquire the qualification:

（1）Register and activate your account to become an XDAEX user.
...
若您有意申请XDAEX平台的程序化交易接入，您需按照以下步骤获得接入资格：

（1）注册并激活，成为XDAEX交易所用户；
...
在您成功激活账户后，您需要在用户中心界面内按照提示进行KYC1级认证，并开通2FA验证。XDAEX运营人员会在其工作时间内及时协助您完成KYC1级认证。

## 4. Apply for API Qualification<br \>&emsp;^申请程序化交易资格
After completing the above steps, you need to send an email request to support@xdaex.com.
...
When completing these documents, you need to provide few basic information to confirm your account ownership and your aim of using our API service. Also, by using XDAEX services, we want to ensure that you understand the risks, scope and limitations of using our API to any 3rd party software or platform.
...
完成上述步骤后，需要您发送邮件到support@xdaex.com进行申请。
...
To help you get familiar with the XDAEX connector system, as well as to maintain your trade rights and other XDAEX users, we have prepared a staging environment which has the same function and connector of the actual production version for testing.

为了便于您熟悉XDAEX的交易接口系统，同时也为了维护您和其他XDAEX用户的交易权益，我们准备了与生产环境版本、功能与接口相同的联调测试环境供您测试。
...
目前，XDAEX常规的维护窗口为每周一、三、五的2:00PM~ 3:00PM（UTC+8）。在这段期间内，我们可能进行版本同步、账户充值等维护工作，这将可能对您在测试环境访问业务的连续性受影响。

### 5.2. Access to the Staging Environment<br \>&emsp;^联调测试接入
You can access the staging environment through the domain api-test.xdaex.com. Please refer to the email with your account name and initial password.

If your host is unable to easily resolve the domain name, you can bind api-test.xdaex.com with
...
您可以通过前述的用户名和初始密码api-test.xdaex.com这一站点来登录测试联调环境。

如果您的主机无法进行域名解析，您可以通过hosts设置来绑定api-test.xdaex.com与45.249.244.103，然后再进行访问请求。在开始测试前，还需要完成以下前置条件：
...
Finally, thank you for choosing XDAEX!
